# Branching Strategy & Operational Instructions (Updated for master)
...
- Short‑lived feature branches derived from phase branches or master.
...
3. Is this user prompt requiring a logged change? → `prompt/<id>-<slug>` (base: active phase or master).
4. Is this purely docs? → `docs/<topic>`.
5. Is this an urgent fix on master? → `fix/<issue>`.
...
### 7.1 General Rule
- Rebase onto updated master or phase branch before PR merge.
...
3. If conflict in `chatlog/index.md`:
   - Open the latest master version.
...
3. `git checkout -b prompt/<id>-<slug> <base>` (base = active phase or master).
...
| Rebase | `git fetch origin && git rebase origin/master` |
| Fast-forward merge (after PR) | `git checkout master && git merge --ff-only prompt/012-add-chunk-merger` |
